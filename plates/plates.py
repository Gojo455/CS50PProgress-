...
to request a vanity license plate for your car, with your choice 
of letters and numbers instead of random ones. Among the requirements, 
though, are:
...
it does not. Assume that s will be a str. You’re welcome to implement additional 
...
def is_valid(s):
    f = stripsspace(s) 
    return validtwoletters(f) and validamt(f)  and validinno(f)
def validtwoletters(x):
...
def validinno(j):
     seenno = False
     for char in j:
          if seenno == False and char == "0":
               return False
          elif char.isalpha() and seenno == True:
               return False
